...
const edited = Paragraph.rehydrate(base);
edited.approve({ content:"approved edited content",theme:"technology",tags:["tech"],reviewerId:"a",reviewedAt:"later",expectedVersion:0 });
assert.equal(edited.content.value,"approved edited content");
assert.equal(edited.theme.value,"technology");
assert.deepEqual(edited.tags.values,["tech"]);

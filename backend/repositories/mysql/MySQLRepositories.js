...
class MySQLParagraphRepository {
  ...
  async create(paragraph) {
    ...
    for (const tagId of paragraph.tags) {
      await this.db.query("INSERT IGNORE INTO paragraph_tags (paragraph_id, tag_id) VALUES (?, ?)", [paragraph.id, tagId]);
    }
  ...
  async deleteById(id) {
    await this.db.query("DELETE FROM paragraph_tags WHERE paragraph_id = ?", [id]);
    await this.db.query("DELETE FROM paragraphs WHERE id = ?", [id]);
  }
